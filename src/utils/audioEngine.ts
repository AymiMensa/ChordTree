...
export class AudioEngine {
  ...
  private scheduleNote(step16: number, time: number) {
    ...
    if (
      ...
      if (this.soundMode === "pad" && beatNumber === 1) {
        ...
        // Calculate strictly ascending frequencies for root position voicing
        const ascendingFreqs: number[] = [];
        let currentFreq = 0;
        for (let i = 0; i < notesToPlay.length; i++) {
          const note = notesToPlay[i];
          let f = NOTE_BASE_FREQS[note] || 261.63;
          // Root note starts one octave lower
          if (i === 0) f *= 0.5;
          while (f <= currentFreq) {
            f *= 2.0;
          }
  ...
  private playChordPad(notes: string[], time: number, duration: number) {
    ...
    // Build strictly ascending frequencies for root position voicing
    // Root is always the lowest, then 3rd, 5th, 7th, etc. ascending
    const ascendingFreqs: number[] = [];
    let prevFreq = 0;
    for (let i = 0; i < notes.length; i++) {
      const note = notes[i];
      let baseFreq = NOTE_BASE_FREQS[note] || 261.63;
      // Root note starts one octave lower
      if (i === 0) baseFreq *= 0.5;
      // Ensure strictly ascending: if current freq is below previous, shift up octaves
      while (baseFreq <= prevFreq) {
        baseFreq *= 2.0;
      }
      ascendingFreqs.push(baseFreq);
      prevFreq = baseFreq;
    }

    notes.forEach((note, idx) => {
      if (!this.ctx) return;
      const frequency = ascendingFreqs[idx];
      const isRoot = idx === 0;
