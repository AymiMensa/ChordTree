...
import { ChordTreeNode } from '../chordsData';
import { ProgressionStep } from '../types';
import { ZoomIn, ZoomOut, Maximize2, RotateCcw, HelpCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
export const ChordMindMapB: React.FC<ChordMindMapProps> = ({ 
  rootTree, 
  maxTreeDepth, 
  collapsedNodes, 
  activeNodeId, 
  ...
  useEffect(() => {
    ...
    // 2. Setup D3 Radial Tree layout for Variant B
    // We use a custom separation for the perfectly symmetric layout of Variant B
    const radiusStep = dimensions.width < 640 ? 70 : 100;
    const treeLayout = d3.tree<ChordTreeNode>()
      .size([2 * Math.PI, maxTreeDepth * radiusStep])
    ...
    if (interactionMode === "fold") {
        nodeG.filter((d: any) => d.data.children.length > 0)
            .append('circle')
            .attr('r', d => d.data.type === 'dominant' ? 20 : (d.depth === 0 ? 32 : 26))
            .attr('fill', 'none')
            .attr('stroke', '#ec4899')
            .attr('stroke-width', 2)
    ...
    nodeG.append('circle')
      ...
      .attr('r', d => d.data.type === 'dominant' ? 16 : (d.depth === 0 ? 28 : 22))
      .attr('fill', 'none')
      .attr('stroke', 'none')
      .attr('stroke-width', 3);
    ...
    nodeG.append('circle')
      ...
      .attr('r', d => d.data.type === 'dominant' ? 12 : (d.depth === 0 ? 24 : 18))
      .attr('fill', d => {
         const isPath = activeProgressionIds.has(d.data.id);
         if (d.data.type === 'dominant') return isPath ? '#451a03' : '#1a140f'; // Brown for dominant
         return isPath ? '#881337' : '#240a13';
      }) 
      .attr('stroke', d => {
         const isPath = activeProgressionIds.has(d.data.id);
         if (d.data.type === 'dominant') return isPath ? '#f59e0b' : '#c78642'; // Yellow/Orange border for dom
         if (d.data.type === 'minor') return isPath ? '#0ea5e9' : '#4eb8df'; // Blue for minor
         return isPath ? '#f43f5e' : '#df4e7e'; // Pink for major
      })
      .attr('stroke-width', d => d.data.type === 'dominant' ? (activeProgressionIds.has(d.data.id) ? 2 : 1.2) : 2);
    ...
    nodeG.append('text')
      ...
      .attr('fill', d => {
        if (d.data.type === 'dominant') return '#c78642';
        return d.data.type === 'minor' ? '#8fd6f5' : '#f58fb3';
      })
      .style('font-size', d => d.data.type === 'dominant' ? '10px' : (d.depth === 0 ? '14px' : '12px'))
      .style('font-weight', '600')
      .style('font-family', d => d.data.type === 'dominant' ? 'ui-monospace, SFMono-Regular, monospace' : 'sans-serif')
      .text(d => d.data.name);

    // Collapsed Subtree Indicator
    nodeG.filter((d: any) => d.data.children && d.data.children.length > 0 && collapsedNodes.has(d.data.id))
      .append('circle')
      .attr('r', d => d.data.type === 'dominant' ? 15 : (d.depth === 0 ? 27 : 21))
      .attr('fill', 'none')
      .attr('stroke', d => d.data.type === 'minor' ? '#4eb8df' : (d.data.type === 'dominant' ? '#c78642' : '#df4e7e'))
      .attr('stroke-width', 1)
      ...
      <div className="absolute top-2 right-0 z-20 w-max max-w-[240px] md:max-w-[280px] bg-[#060a1f]/90 backdrop-blur-md border border-indigo-900/50 border-r-0 rounded-l-xl pl-3 pr-1 py-2 md:pl-4 md:pr-2 md:py-3 shadow-2xl select-none origin-top-right transform scale-75 xl:scale-100 mobile-guide transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            <span className="whitespace-nowrap">對稱放射版 (Tree Mode)</span>
          </div>
