...
export const ChordTreeSvg: React.FC<ChordTreeSvgProps> = ({
  ...
  const { nodeCoords, optimalScale } = useMemo(() => {
    ...
    const normalNodes = CHORD_NODES.filter((n) => n.level > 0);
    let minNormalDistanceSq = minNormalDistance * minNormalDistance;

    for (let i = 0; i < normalNodes.length; i++) {
      const n1 = normalNodes[i];
      const p1 = coords[n1.id];
      if (!p1) continue;
      for (let j = i + 1; j < normalNodes.length; j++) {
        const n2 = normalNodes[j];
        // Radial pruning: if levels differ by more than 1, they are not closest neighbors
        if (Math.abs(n1.level - n2.level) > 1) continue;
        const p2 = coords[n2.id];
        if (!p2) continue;
        const dx = p1.x - p2.x;
        const dy = p1.y - p2.y;
        const distSq = dx * dx + dy * dy;
        if (distSq > 0.01 && distSq < minNormalDistanceSq) {
          minNormalDistanceSq = distSq;
        }
      }
    }
    minNormalDistance = Math.sqrt(minNormalDistanceSq);
  ...
  }, [dimensions, CHORD_NODES, CHORD_LINKS, maxDepth, nodeCoords, viewLimit, baseScale, visitedNodes, visitedLinks, plannedNodes, plannedLinks, activeLinkId, activeNodeId, activeStepIndex, activeProgression, collapsedNodes, interactionMode, metronomeBeat, isPlaying]);
  ...
  const handlePointerDown = (e: React.MouseEvent | React.TouchEvent) => {
    // Don't preventDefault here - it can cause click delays on some browsers
    ...
    for (const node of CHORD_NODES) {
      ...
      // Increase hit area for easier clicking, especially for dominant chords
      const hitRadius = Math.max(radius + 25, minTouchRadius);
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    // Only prevent default for actual right-clicks, not for touch events
    if (!("touches" in e)) {
      e.preventDefault();
    }
    handlePointerDown(e);
