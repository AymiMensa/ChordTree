    ...
    <div className="fixed inset-0 z-50 bg-slate-950 text-white flex flex-col overflow-hidden animate-in fade-in duration-300">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between p-2 sm:p-4 bg-transparent pointer-events-none">
        <div className="flex items-center gap-2 sm:gap-3 pointer-events-auto">
          <div className="p-1 sm:p-2 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg sm:rounded-xl shadow-lg shadow-indigo-500/20 landscape:scale-75 origin-left">
            <Music className="w-3 h-3 sm:w-6 sm:h-6 text-white" />
          </div>
          <div className="landscape:scale-75 origin-left">
        ...
        <Tooltip content="關閉隨堂考試">
          <button 
            ...
            className="p-1.5 sm:p-2 rounded-full hover:bg-white/10 transition-colors pointer-events-auto landscape:scale-75 origin-right"
          >
      ...
      {/* Main Content Area */}
      <div className={`relative z-10 flex-1 flex flex-col landscape:flex-row items-center justify-center p-1 sm:p-2 min-h-0 overflow-hidden pt-12 landscape:pt-0`}>
        
        {/* Chord Flow Display */}
        <div className={`flex flex-row items-center justify-center gap-2 sm:gap-6 md:gap-12 w-full max-w-5xl flex-1 min-h-0 landscape:pr-[120px]`}>
          
          {/* Previous Chord */}
          <div className="flex-1 flex justify-end opacity-40 scale-75 md:scale-90 transition-all duration-300 blur-[1px]">
            {prevChord && (
              <div className={`font-bold text-slate-500 whitespace-nowrap transition-all duration-300 ${showPiano ? 'text-xl sm:text-2xl md:text-4xl landscape:text-xs' : 'text-2xl sm:text-4xl md:text-6xl landscape:text-lg'}`}>
                {prevChord}
              </div>
            )}
          </div>
          ...
          <div className="flex-none flex flex-col items-center justify-center min-w-[100px] sm:min-w-[150px] md:min-w-[250px] shrink-0">
            <div className={`font-black transition-all duration-300 drop-shadow-[0_0_30px_rgba(255,255,255,0.3)]
                ${showPiano ? 'text-4xl sm:text-5xl md:text-7xl landscape:text-2xl' : 'text-5xl sm:text-7xl md:text-[9rem] lg:text-[10rem] landscape:text-4xl'}
                ${isPlaying ? 'text-white scale-110' : 'text-slate-200'}`}>
              {currentChord}
            </div>
            <div className={`mt-1 px-2 sm:px-4 py-0.5 sm:py-1 rounded-full bg-indigo-900/30 border border-indigo-500/30 text-indigo-300 font-bold tracking-widest whitespace-nowrap transition-all duration-300 ${showPiano ? 'text-[5px] sm:text-[6px] landscape:text-[4px] landscape:px-1.5 landscape:py-0' : 'text-[6px] sm:text-[8px] md:text-[10px] md:mt-4 landscape:text-[5px] landscape:mt-1'}`}>
              目前和弦
            </div>
          </div>
          ...
          <div className="flex-1 flex justify-start opacity-80 scale-90 md:scale-100 transition-all duration-300">
            {nextChord && (
              <div className={`font-bold text-pink-500 drop-shadow-[0_0_15px_rgba(236,72,153,0.4)] whitespace-nowrap transition-all duration-300 ${showPiano ? 'text-xl sm:text-3xl md:text-5xl landscape:text-xs' : 'text-3xl sm:text-5xl md:text-7xl landscape:text-lg'}`}>
                {nextChord}
              </div>
            )}
          </div>
        ...
        <div className={`flex flex-col items-center bg-slate-900/60 backdrop-blur-xl rounded-2xl md:rounded-3xl border border-white/10 shadow-2xl w-full max-w-xl shrink-0 transition-all duration-300 
           landscape:absolute landscape:right-2 landscape:top-10 landscape:w-auto landscape:min-w-[120px] landscape:z-20 landscape:p-1.5 landscape:gap-1.5 landscape:scale-90 landscape:origin-top-right
      ...
      <div className={`transition-all duration-500 ease-in-out border-t border-indigo-900/50 bg-slate-950 ${showPiano ? 'translate-y-0 opacity-100 mb-0' : 'translate-y-full opacity-0 absolute bottom-0 left-0 right-0 h-0 overflow-hidden'}`}>
        <div className="p-2 sm:p-4 landscape:p-1 max-w-5xl mx-auto scale-90 sm:scale-100 landscape:scale-[0.80] origin-bottom">
          <PianoVisualizer 
