...
export const ChordMindMap: React.FC<ChordMindMapProps> = ({ 
  ...
  useEffect(() => {
    ...
    const zoom = d3.zoom()
      .scaleExtent([0.15, 4])
      // Only zoom on empty background clicks — let chord nodes
      // receive their own click events without being intercepted.
      .filter((event) => {
        const target = event.target as Element;
        return !target.closest || (!target.closest('.main-node') && !target.closest('.dom-node'));
      })
      .on('zoom', (event) => {
    ...
    domG.append('circle')
      ...
      .on('click', (event, d) => {
        ...
        onNodeClick(d.id, d.name, d.path);
      });
