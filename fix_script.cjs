...
// The duplicate generatePathToNode comment
code = code.replace('// Generate the complete progression steps from Central C to target node\n// Generate the complete progression steps from Central C to target node', '// Generate the complete progression steps from Central C to target node');

const startIdx = code.indexOf('function generateRandomProgression(');
const appComponentIdx = code.indexOf('export default function App() {');

const newFunc = `function generateRandomProgression(
  startNodeId: string,
...
code = code.substring(0, startIdx) + newFunc + code.substring(appComponentIdx);
